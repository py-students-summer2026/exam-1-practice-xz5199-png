...
def bark():
    ...
    age = input("Please enter the age of your dog: ")
    breed = input("Please enter the breed of your dog: ")
    standard_n = name.capitalize()
    standard_b = breed.capitalize()
    print(f"{standard_n}, the {age} year old {standard_b}, says, 'Woof!'")
...
def bark_with_validation():
    ...
    if not (len(name) >= 2 and name.isalpha()):
      print("Name error!")
      return
    if not age.isdigital():
      print("Age Error!")
      return
    valid_b = ["Schnauzer", "Terrier", "Poodle", "Mastiff"]
    if breed.capitalize() not in valid_b:
      print("Breed error!")
      return
    standard_n = name.capitalize()
    standard_b = breed.capitalize()
