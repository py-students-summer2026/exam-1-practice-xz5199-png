...
def get_guess(max_value):
    ...
    guess = input(f"Guess a number between 1 and {max_value}: ")
    try:
        guess_int = int(guess)
    except ValueError:
        return -1
    if guess_int < 1 or guess_int > max_value:
        return -1
    return guess_int == random_number
...
def play_game():
    ...
    if result == True:
        ...
        print("Wrong!")
    percentage = int((correct_count / total_guesses) * 100)
